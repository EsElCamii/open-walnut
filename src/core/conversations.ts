 ...
 * is done. The legacy single-file chat history is COPIED (never deleted) into
 * conversation #1 the first time an agent's index is created.
 */
...
export async function migrateIfNeeded(agentId: string): Promise<void> {
  ...
  await withIndexLock(agentId, async () => {
    ...
    if (legacy && Array.isArray(legacy.entries) && legacy.entries.length > 0) {
      // Copy legacy content into conversation #1 (do NOT delete legacy file).
...
export async function renameConversation(agentId: string, conversationId: string, title: string): Promise<ConversationMeta> {
  validateConversationId(conversationId);
  await migrateIfNeeded(agentId);
  return withIndexLock(agentId, async () => {
    const index = await readIndex(agentId);
